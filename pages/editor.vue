...
const { injectIframeContent, triggerAutosave, setupEditorWatches } = useIframeEngine()
...
onMounted(async () => {
  ...
  ;(window as any).openLinkPrompt = (callback: (val: string) => void) => {
    openPrompt('Insertar Enlace', 'URL del hipervínculo:', 'https://', 'text', callback)
  }
  ;(window as any).openColorPrompt = (callback: (val: string) => void) => {
    openPrompt('Color de Selección', 'Elige un color para el texto resaltado:', '#000000', 'color', callback)
  }
