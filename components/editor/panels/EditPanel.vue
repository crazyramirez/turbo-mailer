      ...
      <div class="control-group">
        ...
        >
          ...
          <template v-if="selectedElement.dataset.type !== 'Texto'">
            ...
            <div
              v-if="selectedElement.dataset.type === 'Precios'"
              class="toggle-item"
            >
              <span class="label">{{ $t('editor.edit_pricing_features') }}</span>
              <button
                @click="toggleVisibility('pricing-features')"
                :class="{ active: isVisible('pricing-features') }"
              >
                <Eye v-if="isVisible('pricing-features')" :size="14" />
                <EyeOff v-else :size="14" />
              </button>
            </div>
